...
- **Logo — IN EXPLORATION (decision pending).** Rounds so far: (1) "id" monogram variants — rejected; (2) literal "dinero on a screen" marks (browser+coin, cash-box monitor, cha-ching phone) — rejected as not hitting; (3) wider net across four corners — hidden-meaning, fintech glyph, growth gesture, minted emblem. **Round-3 candidates** (see exploration board, version `round-3-wider-net`): R1 growth-d (negative-space up arrow), **R2 "đ" invented currency symbol (strongest story)**, R3 rising-bars badge, R4 growth stroke, R5 đ minted on a coin face, R6 window-inside-d. Exploration board: https://claude.ai/code/artifact/4280079b-aa87-42e6-8321-93dcad76dcd8 — user to pick/react before build; wordmark treatment (below) is locked regardless.
